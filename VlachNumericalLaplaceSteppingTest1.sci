...
M      = 2;
N      = 2;
B(1:M) = [5,1];    // B(1)+B(2)*s+...+B(M)*s**(M-1)
A(1:N) = [1,0.5];   // A(1)+A(2)*s+...+A(N)*s**(N-1) + "s**N"
...
// Time:
h      =  0.01; // Stepsize for numerical TF
TEND   = 20.0;  // End time
...
wurzel=sqrt(15)/4.0;
plot(xt,5+0.5*(exp(-xt/4).*(-2*sin(wurzel*xt)/sqrt(15)-10*cos(xt*wurzel))),'ro');
